(function ($) {
    ...
    return $.ajax({
      ...
      success: function (data, textStatus, jqXHR) {
        if (data.success == 1) {
          ...
          $(".profile-section__users-left").text(
            usercount - 1 + " of " + total + " used."
          );
        } else {
          var text = "";
  ...
  Acme.UserProfileController.prototype.renderUser = function (
    parent,
    data,
    template
  ) {
    var userTemp = template
      ...
      : Handlebars.compile(window.templates.managed_user);
  ...
  Acme.UserProfileController.prototype.events = function () {
    ...
    $("#addManagedUser").on("click", function (e) {
      ...
      var html =
        '<li id="newUser" class="user-editor">' + userTemp(data) + "</li>";
    ...
    $("#cancelAccount").on("click", function (e) {
      ...
      var message =
        "Are you sure? Click OK to deactivate your subscription to frank.";
      var status = "cancelled";
      if ($(e.target).text() == "Restart Subscription") {
        //template = "userPlanChange";
        message =
          "Click OK to reactivate your plan. Your credit card will be charged on the next payment date.";
        status = "paid";
      }
      var requestData = {
        status: status,
      ...
      Acme.SigninView.render(template, message).done(function () {
      ...
      if (Number(usercount) <= Number(planusers)) {
        ...
        if ((newplandailycost - plandailycost) * diffDays >= 0) {
          msg =
            " This will cost " +
            currencySymbol +
            Math.round((newplandailycost - plandailycost) * diffDays);
          msg = msg.replace(/(.+)(\d\d)$/g, "$1.$2");
        }

        Acme.SigninView.render(
          "userPlanChange",
          "Are you sure you want to change plan?" + msg
        ).done(function () {
          $("#dialog").parent().remove();
          ...
          $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
              ...
              if (data.success == 1) {
                ...
                var text = "";
                if (
                  Object.prototype.toString.call(data.error) !==
                  "[object Array]"
                ) {
                  var err = [data.error];
                  data.error = err;
                ...
                Acme.SigninView.render("userPlanAuth", text).then(function (r) {
                  ...
                  auth.addEventListener("click", function (event) {
                    stripe
                      .confirmCardPayment(data.error_data.client_secret, {
        ...
        Acme.SigninView.render(
          "userPlan",
          "You have too many users to change to that plan."
        );
      }
    });
  };
    ...
    $(".j-deleteListing")
      ...
      .on("click", function (e) {
        ...
        Acme.SigninView.render(
          ...
          Acme.server
            .create("/api/article/delete-user-article", { articleguid: id })
            .done(function (r) {
              listing.remove();
            })
            .fail(function (r) {
